...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors'), changes=changes).to(device)  # create
        ...
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors'), changes=changes).to(device)  # create
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            for p in qat_model.parameters():
                print(p, "LLLLLLLLLL")
            j = deepcopy(qat_model)
            qat_model.eval()
            for p in qat_model.parameters():
                print(p, "LLLLLLLLLL")
            quantized_model = torch.quantization.convert(qat_model.eval(), inplace=False)
            print("HI")
            # return quantized_model, j
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                #####THIS IS BROKEN THIS IS BROKEN THIS IS BROKEN THIS IS BROKEN THIS IS BROKENTHIS IS BROKENTHIS IS BROKEN THIS IS BROKEN THIS IS BROKEN THIS IS BROKEN
                # ckpt = {'model': quantized_model,'optimizer' :optimizer.state_dict()}
...
def parse_opt(known=False):
    ...
    parser.add_argument('--changes', type=str, default=None, help='Changes to make')
...
def main(opt, callbacks=Callbacks()):
    ...
    # Train
    if not opt.evolve:
        return train(opt.hyp, opt, device, callbacks)
